  ...
  Widget build(BuildContext context) => Scaffold(
        body: Center(
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [
            Icon(Icons.school, size: 120, color: green),
            SizedBox(height: 18),
            Text('Ruralz - Maths', style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
            Text('Developed by D. K. Chavan sir', style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 6),
        ...
        body: FutureBuilder<List<dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              return ErrorPanel(title: 'Unable to load topics', error: snapshot.error.toString(), onRetry: retry);
            }
            final list = snapshot.data ?? [];
            if (list.isEmpty) {
              return ErrorPanel(title: 'No topics found', error: 'The request succeeded but returned zero topics.', onRetry: retry);
            }
            return ListView(
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: FutureBuilder<List<dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) return ErrorPanel(title: 'Unable to load tests', error: snapshot.error.toString(), onRetry: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => Tests(topicId: topicId, name: name))));
            final list = snapshot.data ?? [];
...
class ErrorPanel extends StatelessWidget {
  ...
  const ErrorPanel({super.key, required this.title, required this.error, required this.onRetry});
  ...
  Widget build(BuildContext context) => Center(
        child: SingleChildScrollView(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 12),
            SelectableText(error, textAlign: TextAlign.center, style: const TextStyle(fontSize: 12)),
            const SizedBox(height: 16),
